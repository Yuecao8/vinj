<template>
  <div class="wrapper" v-if="token">
    ...
    <div class="img-box flexBox flexWrap" :class="{'pd15':imgList.length}">
      <p v-if="imgList.length" style="width:100%;" class="tips">长按图片可拖动排序</p>
      <!-- 图片可拖动排序 -->
      <draggable v-model="imgList">
        <transition-group class="flexBox flexWrap">
          <div
            ...
            :key="item.id"
          >
            <!-- <img class="main-img" :src="item.upLoadData.photo_url" alt /> -->
      ...
      <!-- <div
        class="img-item"
        :class="{'ml0':imgList.length % 3 === 0}"
        style="margin-left:0.04rem;"
        @click="selectPic"
      >
        <img class="main-img" src="../assets/images/contribute/add_big.png" alt />
      </div>-->
      <!-- 图片input -->
      <input
        v-if="isShowInput"
    ...
    <div class="main">
      ...
      <div class="info-item flexBox" @click="openTags">
        <p class="left">
          标签
          <i></i>
        </p>
    ...
    <mt-popup v-model="coverVisible" popup-transition="popup-fade" class="cover-mask">
      ...
      <div class="swiper-box">
        <swiper :options="swiperOption" ref="mySwiper" v-if="imgList.length && coverVisible">
          <swiper-slide v-for="(item,index) in imgList" :key="index">
            <img style="width:100%;" :src="item.upLoadData.photo_url" alt />
          </swiper-slide>
        </swiper>
      </div>
      <div class="cover-bottom" v-if="imgList.length && coverVisible">
        ...
        <div class="set-box flexBox flexAlign">
          <img
            ...
            alt
          />
          <img v-else @click="setCover(0)" src="../assets/images/contribute/set_covered.png" alt />
          <p>设为封面</p>
        </div>
      </div>
    </mt-popup>
...
export default {
  ...
  methods: {
    ...
    addTags () {
      // 去空格
      let regEx = /\s+/g
      this.addingTag = this.addingTag.replace(regEx, ' ')
      // 去收尾空格
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        let config = {
          ...
          onUploadProgress: progressEvent => {
            ...
            if (files.length === 1) {
              _this.imgList[nowIndex].progress = complete
            } else {
              _this.imgList[nowIndex + i].progress = complete
            }
          }
        }
        axios.post('http://plus-middle-end.dev.hoge.cn/api/gallery/upload', formData, config).then(res => {
    ...
    // 设为封面
    setCover (flag) {
      this.ifCoverSet = !this.ifCoverSet
      if (this.ifCoverSet) {
        let formData = new FormData()
        formData.append('access_token', this.token)
    ...
    publish () {
      ...
      let param = {
        ...
        keywords: this.tagList.length ? this.tagList.join(',') : '',
        cover_type: 1,
      ...
      // if (this.coverList.length) {
      //   param.cover_type = 1
      //   param.cover_keys = JSON.stringify(this.coverList)
      // }
      // 有拍摄时间
      if (this.time) {
