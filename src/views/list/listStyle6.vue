...
// cssid 9910 双列两图
<template>
  <div class="wrapper" v-wechat-title="this.title+'_视觉南京'" v-if="navList.length > 1">
    <!-- 底部 -->
    <header>
    ...
    <div class="container p15">
      <div class="item-box" v-for="(item,index) in navList.slice(1,navList.length)" :key="index">
        ...
        <div v-if="cssid == 9906">
          ...
          <div class="imgs-box mb10" v-if="item.data.length">
            <div class="large-img flexBox flexMiddel" @click="goDetail(item.data[0])">
              <img class="fullImg" :src="item.data[0].index_pic+'?imageView/0/h/254/w/345'" alt />
            </div>
            <div class="img-breif">
        ...
        <!-- 四小图 -->
        <div class="flexBox flexBetween flexWrap" v-if="cssid == 9907 || cssid == 9908">
          <div
            class="img-item"
            :class="{'mt10':index < 2}"
