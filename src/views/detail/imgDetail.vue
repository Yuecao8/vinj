<template>
  <div class="wrapper" v-wechat-title="this.detail.title+'_视觉南京'" v-if="detail.id">
    <detail-top-img
      ...
      :imgUrl="detail.index_pic+'?imageView/0/h/250/w/375'"
      :type="'img'"
      :id="id"
      :title="detail.title"
      :collected="ifCollect"
    ...
    <div class="info-box">
      ...
      <div class="work-box">
        ...
        <div class="work-item flexBox flexAlign" v-if="detail.shoot_time">
          <p class="sub-title">拍摄时间</p>
          <p
            class="detail"
          >{{detail.shoot_time !== '' ? detail.shoot_time.substring(0,16) : detail.created_at.substring(0,16)}}</p>
        </div>
      </div>
    </div>
    <!-- 登录网页端评分 -->
